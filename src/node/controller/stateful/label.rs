...
impl Label {
                ...
                match sel.ready() {
                    ...
                    _ => {
                        ...
                        match self.map.remove(&label) {
                            None => match self.default_fun {
                                Some(fun) => {
                                    thread_pool_channel
                                        .send(ThreadMessage::new_with_stateful(
                                            fun,
                                            msg,
                                            None,
                                            self.fn_sender.clone(),
                                        ))
                                        .unwrap();
                            ...
                            Some(option_fun) => match option_fun {
                                Some(fun) => {
    ...
    fn receive_handler(
        &mut self,
        thread_pool_channel: Sender<ThreadMessage>,
        option_label: Option<String>,
        fun: Box<FnMut(Vec<u8>) + Send + Sync + 'static>,
    ) {
        let optional_vec = self
            .queue_by_label
            .get_mut(&option_label)
            .unwrap()
            .dequeue();

        match option_label {
            None => match optional_vec {
                None => self.default_fun = Some(fun),
                Some(vec) => {
                    thread_pool_channel
                        .send(ThreadMessage::new_with_stateful(
                            fun,
    ...
    pub fn add_handlers(
        &mut self,
        list: Vec<(String, Box<FnMut(Vec<u8>) + Send + Sync + 'static>)>,
    ) {
        for (l, f) in list {
